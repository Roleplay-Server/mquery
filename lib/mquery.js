...
Query.prototype.setOptions = function(options) {
  ...
  var methods = utils.keys(options),
    ...
    var conditionals = utils.keys(this._conditions);

  for (var i = 0; i < methods.length; ++i) {
    method = methods[i];

    /*if (false && this._pipeline)
    {
      switch(method)
      {
          case "sort":
            this._pipeline.push({$sort: {[Object.keys(options[method])[0]]: options[method][Object.keys(options[method])[0]]}});
            break;
          case "limit":
            this._pipeline.push({$limit: options[method]});
            break;
          case "skip":
            this._pipeline.push({$skip: options[method]});
            break;
      }
    }*/

    var pipeline_backup = this._pipeline.slice();
  ...
  this._pipeline = pipeline_backup.slice();

  /*for (var i = 0; i < conditionals.length; i++) {
    var variable = conditionals[i];
    var sub_conditional_list = {};

    var sub_conditionals = utils.keys(this._conditions[variable]);

    for (var k = 0; k < sub_conditionals.length; k++) {
      var sub_conditional_key = sub_conditionals[k];
      var sub_conditional_value = this._conditions[variable][sub_conditional_key];
      this._pipeline.push({$match: {[variable]: {[sub_conditional_key]: sub_conditional_value}}})
      //sub_conditional_list.push({[sub_conditional_key]: sub_conditional_value});
    }
  }*/
...
Query.prototype.where = function() {
  ...
  if ('object' == type && !Array.isArray(arguments[0])) {
    var keys = Object.keys(arguments[0]), key;

    for (var i = 0; i < keys.length; i++)
    {
      var key = keys[i];
      this._pipeline.push({$match: {[keys[i]]: arguments[0][keys[i]]}});
    }
...
Query.prototype.eq = function eq(val) {
  ...
  var path = this._path;
  this._conditions[path] = val;
  if (!this._pipeline) this._pipeline = [];
    this._pipeline.push({$match: {[path]: val}})
  return this;
};
...
'gt gte lt lte ne in nin all regex size maxDistance minDistance'.split(' ').forEach(function($conditional) {
  Query.prototype[$conditional] = function() {
    ...
    conds['$' + $conditional] = val;
      if (!this._pipeline) this._pipeline = [];
      this._pipeline.push({$match: {[path]: {["$" + $conditional]: val}}})
    return this;
  };
});
...
Query.prototype.sort = function(arg) {
  ...
  if (Array.isArray(arg)) {
    ...
    for (i = 0; i < arg.length; ++i) {
      ...
      this._pipeline.push({$sort: {[arg[i][0]]: arg[i][1]}});
    }
    return this;
  }
  ...
  if (1 === arguments.length && 'string' == type) {
    ...
    for (i = 0; i < len; ++i) {
      ...
      push(this.options, field, ascend);
        this._pipeline.push({$sort: {[field]: ascend}});
    }
  ...
  if (utils.isObject(arg)) {
    ...
    for (i = 0; i < keys.length; ++i) {
      ...
      push(this.options, field, arg[field]);
        this._pipeline.push({$sort: {[field]: arg[field]}});
    }
...
['limit', 'skip', 'maxScan', 'batchSize', 'comment'].forEach(function(method) {
  Query.prototype[method] = function(v) {
    ...
    {
        case "limit":
            this._pipeline.push({$limit: v});
          break;
        case "skip":
            this._pipeline.push({$skip: v});
          break;
    }
...
Query.prototype.merge = function(source) {
  ...
  if (source instanceof Query) {
    ...
    if (source._pipeline) {
      this._pipeline = source._pipeline;
    }
...
Query.prototype.aggregate = function (pipeline, callback) {
  this.op = 'aggregate';

    if ('function' === typeof pipeline) {
        callback = pipeline;
        pipeline = undefined;
    }
    else if (Query.canMerge(pipeline))
    {
      this.merge(pipeline);
    }
  ...
  options.fields = this._fieldsForExec();
    ...
    debug('aggregate', this._collection.collectionName, pipe, options);
  callback = this._wrapCallback('aggregate', callback, {
      pipeline: pipe
      , options: options
  });

  this._collection.aggregate(pipe, options, utils.tick(callback));
  return this;
}

Query.prototype.sum = function (property, callback) {
    //return this.aggregate([{$group: {_id: null, total: {$sum: property}}}]);
    this.op = 'sum';
    ...
    else
    {
        ...
        this._pipeline.push({$group: {_id: null, total: {$sum: property}}});
    }
    ...
    debug('sum', this._collection.collectionName, pipe, options);
    var cb_replacement = function(err, data) {
        if (err||!data){return callback(err);} else if(data.constructor !== Array) { return callback(err, data);} callback(err, data.length == 0 ? 0 : data[0]['total']);};
    /*callback = this._wrapCallback('sum', cb_replacement, {
        pipeline: pipe
        , options: options
    });*/

    this._collection.aggregate(pipe, options, cb_replacement);
    return this;
}
